...
OPTIONS=(1 "Install needed dependencies for compilation and execution"
         ...
         5 "Install DrWh0's patch / Spanish tranlation (current user only)"
...
case $CHOICE in
        ...
        5)
			...
			dialog --title "Spanish translation for TSX/MOD (ESC to cancel)" \
            ...
            1) 
               rm -Rf ~/.openMSX/share/scripts/*.*
               (pv -n script-normal-new.tar.gz | tar xzf - -C ~/.openMSX) 2>&1 | dialog --gauge "Uncompressing custom modifications.." 6 50
               dialog --infobox "Customization finished...." 3 30 ; sleep 2
               ;;
            255) 
               dialog --infobox "Returning to main menu............" 3 43 ; sleep 2
